...
func (c *Conn) Read(b []byte) (n int, err error) {
	...
	if n > 0 {
		if Debug {
			if n > 0 {
				Debug.Printf("n, err = c.Conn.Read(cipherData)\n")
				DumpMemoryEx(cipherData, n, true)
				Debug.Printf("\n")
			}
		}
		...
		if Debug {
			if n > 0 {
				Debug.Printf("c.decrypt(b[0:n], cipherData[0:n])\n")
				DumpMemoryEx(b, n, true)
				Debug.Printf("\n")
			}
		}
	}
...
func (c *Conn) Write(b []byte) (n int, err error) {
	...
	if Debug {
		if len(iv) > 0 {
			Debug.Printf("copy(cipherData, iv)\n")
			DumpMemoryEx(cipherData, len(iv), true)
			Debug.Printf("\n")
		}
	}
	...
	if Debug {
		if dataSize > 0 {
			Debug.Printf("c.encrypt(cipherData[len(iv):], b)\n")
			DumpMemoryEx(cipherData, dataSize, true)
			Debug.Printf("\n")
		}
	}
	...
	Debug.Printf("n, err = c.Conn.Write(cipherData)\n")
	Debug.Printf("n = %d\n", n)
	Debug.Printf("\n")
	return
}
